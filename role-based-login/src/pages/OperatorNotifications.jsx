        ...
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {notifications.map((notif) => (
            <li key={notif.id} style={{ background: notif.is_read ? '#f1f5f9' : '#fff', margin: '12px 0', padding: '16px', borderRadius: '8px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              <div style={{ fontSize: '0.8em', color: '#666', marginTop: '8px' }}>{new Date(notif.created_at).toLocaleString()}</div>
            </li>
          ))}
        </ul>
      )} */}
      <p style={{ textAlign: 'center', color: '#888' }}>Notifications feature temporarily disabled</p>
    </div>
  );
}
